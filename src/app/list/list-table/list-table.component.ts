...
export class ListTableComponent {
  ...
  readonly displayedColumns = signal<string[]>(['name', 'company', 'type', 'applicationDate', 'decision']);
  ...
  private checkScreenSize(): void {
    ...
    if (isMobileView) {
      ...
      this.displayedColumns.set(['name', 'company', 'type', 'applicationDate', 'decision']);
    }
  }
  ...
  async loadJobs(page: number = 0, pageSize: number = 10): Promise<void> {
    ...
    try {
      ...
      if (this.includeExpired() && this.filterStatus() === 'unknown') {
        ...
        this.totalItems.set(this.totalItems() + expiredResponse.pagination.total);
      }

      this.jobs.set(jobs);
      this.dataSource.data = jobs;
      this.syncPaginatorWithCurrentState();    } catch (error) {
